...
@transaction.atomic
def add_points(user: AppUser, amount: int, reason: str, ref: str | None = None) -> PointTransaction:
    if amount <= 0:
    ...
    txn = PointTransaction.objects.create(
        user=u, kind=PointTransaction.Kind.EARN, amount=amount, reason=reason, reference=ref
    )
...
def deduct_points(user: AppUser, amount: int, reason: str, ref: str | None = None) -> PointTransaction:
    ...
    u = AppUser.objects.select_for_update().get(pk=user.pk)
    u.refresh_from_db(fields=["reward_points"])
    if u.reward_points < amount:
        raise ValueError("Insufficient points")

    txn = PointTransaction.objects.create(
        user=u, kind=PointTransaction.Kind.SPEND, amount=amount, reason=reason, reference=ref
    )
    u.reward_points = F("reward_points") - amount
    u.save(update_fields=["reward_points"])
    u.refresh_from_db(fields=["reward_points"])
